...
private:
    ...
    {
        cv::namedWindow(BoardDisplay::WINDOW);
        cv::imshow(BoardDisplay::WINDOW,img);
        cv::waitKey(msec);
...
public:
    ...
    BoardDisplay() : it_(nh_)
    {
        image_pub_ = it_.advertise("new_board_scheme", 1);
        sub = nh_.subscribe("/new_board", 1, &BoardDisplay::publish_draw_board, this);
    }
...
const char BoardDisplay::WINDOW[] = "Tic Tac Toe Board Status";
}
